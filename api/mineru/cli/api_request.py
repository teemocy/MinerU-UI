...
""",
    ...
    processing_window_size: Annotated[
        ...
        Form(
            description=(
                "Pages per VLM/hybrid processing window. "
                "Defaults to MINERU_PROCESSING_WINDOW_SIZE or the server default; "
    ...
    if client_side_output_generation:
        ...
        return_images = True
    if processing_window_size is not None and processing_window_size <= 0:
        raise HTTPException(
            status_code=400,
            detail="processing_window_size must be a positive integer.",
        )
    processing_window_size = clamp_processing_window_size(processing_window_size)
