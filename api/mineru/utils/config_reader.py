...
def get_vlm_http_max_concurrency(default: int = 8) -> int:
    if default <= 0:
        raise ValueError(
            f"default vlm http max_concurrency must be a positive integer, got {default}"
        )
    value = os.getenv("MINERU_VLM_HTTP_MAX_CONCURRENCY")
    if value is None:
        return default
    try:
        ...
        logger.warning(
            "Invalid MINERU_VLM_HTTP_MAX_CONCURRENCY value: "
            f"{value}, use default {default}"
        )
        return default
